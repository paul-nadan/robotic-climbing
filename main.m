...
SEED = 43; % set the terrain seed, -1 for a random seed
...
elseif ~OPTIMIZE && ~PLOT_ONLY
    [meanScores, rawScores, allScores, seeds, robots] = simulate(@getConfig, ...
        ...
        TIME_STEP, ABORT_STRIKES, IGNORE_FAILS, SEED, SCORES, REUSE_DATA, fixedFootPlacements);
end
...
%         0.1, 0.3, {var1, var2}, 0, 2);
    code = dec2bin(var1) == '1';
    code = [zeros(1,9-length(code)),code];
    config = simpleWalker(code, .2, .6, .3);
end
