...
class Robot:
    ...
    def set_behavior(self, behavior=None, *args, **kwargs):
        """
...
class Leg:
    ...
    def get_force(self, t1=None, t2=None, t3=None, relative=False, goal=False):
        ...
        fx, fy, fz = -fx, -fy, -fz  # applied forces -> reaction forces
        if not relative:
            if self.mirror:
                fx *= -1
    ...
    def get_torque(self, fx=None, fy=None, fz=None, relative=False, goal=False):
        ...
        a1, a2, a3 = self.get_angle()
        fx, fy, fz = -fx, -fy, -fz  # reaction forces -> applied forces
    ...
    def move_in_direction(self, dt, dx=0, dy=0, dz=0, v=None, relative=False, direct=False):
        ...
        :return: True if edge of workspace is reached
        """
        d = np.sqrt(dx ** 2 + dy ** 2 + dz ** 2)
        if d <= 0:
            return
        v_max = self.get_max_speed(dx/d, dy/d, dz/d, relative)
        if v is None or v > v_max:
            # if v is not None and v != 500 and v != 50:
            #     print("TOO FAST!!!!!!!!!")
            #     v[2] = 0
            v = v_max
