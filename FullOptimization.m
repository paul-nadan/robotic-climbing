...
if p.simWorkspace
    workspaces = getWorkspaces(configs, L0, p);
...
for i = 1:length(configs)
    ...
    torque = reshape(getDeviation(torque, p.sigma)/5, [], 1);
    torqueScores(i) = max(torque);
...
%     score = torqueScores(i);
    ...
    if code(1)%any(index == -[3,12,31,41,62,91,74]) % annotate selected designs
