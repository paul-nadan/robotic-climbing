...
// Gets current motor position (deg)
...
// Gets current motor output torque (Nmm)
...
float* getFootForce(int foot) {
  ...
  return(getEndForce(-t1, -t2, -t3, a1, a2, a3));
}

void enableTorque() {
  Dxl.writeByte(1,TORQUE_ENABLE_XM,0);
  Dxl.writeByte(2,TORQUE_ENABLE_XM,0);
  Dxl.writeByte(3,TORQUE_ENABLE_XM,0);
  Dxl.writeByte(4,TORQUE_ENABLE_XM,0);
  Dxl.writeByte(13,TORQUE_ENABLE_XM,0);
  Dxl.writeByte(1,OPERATING_MODE_XM,3);
  Dxl.writeByte(2,OPERATING_MODE_XM,3);
  Dxl.writeByte(3,OPERATING_MODE_XM,3);
  Dxl.writeByte(4,OPERATING_MODE_XM,3);
  Dxl.writeByte(13,OPERATING_MODE_XM,3);
...
// Determine joint angles position (deg) from end effector position (mm)
float* getJointPos(float x, float y, float z) {
  static float a[3];
  a[0] = (1)*180/PI;
  a[1] = (1)*180/PI;
  a[2] = (1)*180/PI;
...
// Determine end effector position (mm) from joint angles (deg)
float* getEndPos(float a1, float a2, float a3) {
  a1 = a1*PI/180;
  a2 = a2*PI/180;
  a3 = a3*PI/180;
  static float x[3];
...
float* getJointVel(float vx, float vy, float vz, float a1, float a2, float a3) {
  a1 = a1*PI/180;
  a2 = a2*PI/180;
  a3 = a3*PI/180;
  static float w[3];
...
float* getEndVel(float w1, float w2, float w3, float a1, float a2, float a3) {
  a1 = a1*PI/180;
  a2 = a2*PI/180;
  a3 = a3*PI/180;
  w1 = w1*PI/180;
...
float* getJointTorque(float fx, float fy, float fz, float a1, float a2, float a3) {
  a1 = a1*PI/180;
  a2 = a2*PI/180;
  a3 = a3*PI/180;
  static float t[3];
...
float* getEndForce(float t1, float t2, float t3, float a1, float a2, float a3) {
  a1 = a1*PI/180;
  a2 = a2*PI/180;
  a3 = a3*PI/180;
  static float f[3];
...
void graspForce() {
  ...
  for(int i = 0; i < 300; i++) {
    da = getJointVel(0, 0, -1, a1, a2, a3);
    t = getJointTorque(0, 0, -5, a1, a2, a3);
...
//  SerialUSB.println(a3);
  ...
  for(int i = 0; i < 600; i++) {
    da = getJointVel(0, -1, 0, a1, a2, a3);
    t = getJointTorque(10, -20, -5, a1, a2, a3);
    limitTorque(2, abs(t[0]));
    limitTorque(6, abs(t[1]));
    limitTorque(10, abs(t[2]));
    a1 += da[0]*v;
