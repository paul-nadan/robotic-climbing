...
%     Aeq = [Aeq, zeros(horizon, length(F0))];
    ...
    options = optimoptions('fmincon','MaxFunctionEvaluations',1e4,...
        ...
        'OutputFcn', @saveBest);
    [xFree,~,flagFree,outputFree] = fmincon(@(x)costFull(x, [], 0),Xn,[],[],...
...
%             drawnow()
    ...
    for iStep = 1:length(weights)
        ...
        g0 = zeros(sum(any(r0.gait.feet==1, 2)),1);
        F0 = [F0; f0];
        C0 = [C0; c0];
        G0 = [G0; g0];
...
%             swingPos = swingPos + sum(r0.gait.dx, 2);
    ...
    lb = [lb; zeros(nFC, 1)-Inf; zeros(nG,1)-pi];
    ub = [ub; zeros(nFC, 1)+Inf; zeros(nG,1)+pi];
    x_optimal = 0;
    f_optimal = -1;
    
    [x,~,flag,output] = fmincon(@(x)costFull(x, weights, nG),[Xn;F0;C0;G0],[],[],...
...
%     end
    ...
    r.skip = skip;
    r.F = reshape(x(end-nFC-nG+1:end-nFC-nG+length(F0)/length(weights)), 3, []);
    r.G = x(end-nG+1:end);
...
function [c, g] = costFull(x, weights, nG)
    horizon = length(weights);
    c = sum(x(end-horizon-nG+1:end-nG).*weights);
    g = zeros(size(x));
    g(end-horizon+1-nG:end-nG) = weights;
end
...
function [c,ceq] = constraintsFull(x, rd, i, stepping, grid, costHorizon, skip, swingPos)    
    ...
    iG = mod(i+skip-1, size(rd(1).gait.angles, 2))+1; % next step in gait cycle
    nF = 3*sum(rd(2).gait.feet(:,iF(1)) > 0); % number of force variables
    nG = sum(any(rd(2).gait.feet==1, 2));
    X = x(1:end-costHorizon*(1+nF+nG)); % state variables
    xF = x(end-costHorizon*(1+nF+nG)+1:end-costHorizon*(1+nG)); % force variables
    xC = x(end-costHorizon*(1+nG)+1:end-costHorizon*nG); % cost variables
    xG = x(end-costHorizon*nG+1:end);
...
%     G = [1;0;0]*9.81;
    ...
    for iJoint = 1:size(robot.vertices, 2)
        ...
        iFeetBool = double(robot.gait.feet(:,iF)' > 0);
    ...
    c = [c; abs(T(1,:))' - robot.config.torqueLimits];
end

function [c, ceq] = constraintsGripper(robot, xG, xG0, xF, iF, iG, grid)
    ifeet = any(robot.gait.feet == 1, 2);
    feet = robot.vertices(:, ifeet);
    istance = robot.gait.feet(:,iG) == 1;
    iforce = robot.gait.feet(:,iF) == 1;
    iswing = any(robot.gait.feet == 1, 2) & (robot.gait.feet(:,iG) ~= 1);
    ...
    F = reshape(xF, 3, []);
    Fg = zeros(size(feet)); % forces on all grippers
    Fg(:,iforce(ifeet)) = F(:,1:sum(iforce)); % TODO fix if tail is not last
    
    Ny = [N(1,:)*0;
          sqrt(1-N(2,:).^2./(N(2,:).^2+N(3,:).^2));
          -abs(N(2,:))./sqrt(N(2,:).^2+N(3,:).^2).*sign(N(3,:))];
    Nx = cross(Ny, N);
    g = diag(atan2(Fg'*Nx, Fg'*Ny)); % 4x1 angle vector
    
    if robot.config.swivelType == 1 % wrist aligned
        r = robot.links(:,any(robot.gait.feet == 1, 2));
        g0 = diag(atan2(r'*Nx, r'*Ny));
    elseif robot.config.swivelType == 2 % body aligned
        r = robot.R0*[1;0;0];
        g0 = 0*xG + atan2(r'*Nx, r'*Ny);
    elseif robot.config.swivelType == 3 % gravity aligned
        g0 = 0*xG;
    end
    g0 = g0 + robot.config.swivelOffset;
    
    c = [abs(xG(iswing(ifeet)) - g0(iswing(ifeet))) - robot.config.swivel1; % swing feet
         abs(xG(iforce(ifeet)) - g(iforce(ifeet))) - robot.config.swivel2]; % force feet
    ceq = xG(istance(ifeet)) - xG0(istance(ifeet)); % stance feet
end
